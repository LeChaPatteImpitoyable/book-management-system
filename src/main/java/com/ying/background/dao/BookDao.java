...
public class BookDao {
    ...
    private final static String QUERY_PAGE_BOOKS_SQL="SELECT * FROM book_info limit ?,?";
    private final static String QUERY_BOOK_SQL="SELECT * FROM book_info WHERE  position(? in book_id)  or position(? in name)   ";
    private final static String QUERY_PAGE_BOOK_SQL="SELECT * FROM book_info WHERE  position(? in book_id)  or position(? in name) limit ?,?  ";
    //查询匹配图书的个数
    private final static String MATCH_BOOK_SQL="SELECT count(*) FROM book_info WHERE position(? in book_id)  or position(? in name)  ";
    private final static String QUERY_ALL_BOOK_COUNT_SQL="SELECT count(*) FROM book_info";
    //根据书号查询图书
    private final static String GET_BOOK_SQL="SELECT * FROM book_info where book_id = ? ";

    public int queryAllBookCount(){
        return jdbcTemplate.queryForObject(QUERY_ALL_BOOK_COUNT_SQL,Integer.class);
    }

    public int matchBook(String searchWord){
        String swcx="%"+searchWord+"%";
        return jdbcTemplate.queryForObject(MATCH_BOOK_SQL,new Object[]{searchWord,searchWord},Integer.class);
    }

    public List<Book> queryBook(String sw){
        final List<Book> books=new ArrayList<Book>();
        jdbcTemplate.query(QUERY_BOOK_SQL, new Object[]{sw,sw}, new RowCallbackHandler() {
    ...
    public List<Book> queryPageBook(String sw, int offset, int length){
        final List<Book> books=new ArrayList<Book>();
        jdbcTemplate.query(QUERY_BOOK_SQL, new Object[]{sw,sw,offset,length}, new RowCallbackHandler() {
            public void processRow(ResultSet resultSet) throws SQLException {
                resultSet.beforeFirst();
                while (resultSet.next()){
                    Book book =new Book();
    ...
    public List<Book> getPageBooks(int offset, int length){
        ...
        jdbcTemplate.query(QUERY_PAGE_BOOKS_SQL,new Object[]{offset, length}, new RowCallbackHandler() {
            public void processRow(ResultSet resultSet) throws SQLException {
                resultSet.beforeFirst();
                while (resultSet.next()){
                    Book book =new Book();
